...
(function () {
  ...
  const LIBRARY_INDEX_PATH = "data/library-index.json";
  let staticSongIndexPromise = null;

  function normalizeStaticSongItem(item) {
    if (!item || typeof item !== "object") return null;
    const filename = safeJsonFilename(item.filename || String(item.path || "").split("/").pop(), "Cancion");
    const rawPath = String(item.path || "").replace(/^\.?\//, "");
    const path = rawPath || `library/${filename}`;
    if (!/^(library|library2)\//.test(path) || !path.toLowerCase().endsWith(".json")) {
      console.warn("ATA: entrada de biblioteca ignorada por ruta invalida:", item);
      return null;
    }
    return {
      filename,
      title: String(item.title || titleCaseFilenameBase(filename)).trim() || titleCaseFilenameBase(filename),
  ...
  async function staticSongMetadata() {
    if (!staticSongIndexPromise) {
      staticSongIndexPromise = (async () => {
        const raw = await staticJson(LIBRARY_INDEX_PATH, []);
        const items = Array.isArray(raw) ? raw : Array.isArray(raw?.songs) ? raw.songs : [];
        const normalized = items.map(normalizeStaticSongItem).filter(Boolean);
        const seen = new Set();
        const duplicates = new Set();
        normalized.forEach(item => {
          const key = item.filename.toLocaleLowerCase("es");
          if (seen.has(key)) duplicates.add(item.filename);
          seen.add(key);
        });
        if (duplicates.size) {
          throw new Error(`Hay nombres de archivo duplicados entre library y library2: ${[...duplicates].join(", ")}`);
        }
        return normalized;
      })();
    }
    return structuredClone(await staticSongIndexPromise);
  }

  async function findStaticSong(filename) {
    const songs = await staticSongMetadata();
    return songs.find(item => item.filename === filename) || null;
  }

  async function loadStaticSong(filename) {
    const item = await findStaticSong(filename);
    if (!item) return null;
    const response = await nativeFetch(staticUrl(item.path));
    if (!response.ok) return null;
    return response.json();
  }

  async function listSongs() {
    const [staticItems, records] = await Promise.all([
      staticSongMetadata(),
      recordsOfType("song")
    ]);

    // La biblioteca incluida es la base. Un guardado privado con el mismo
    // filename actua como override local y se puede eliminar para volver
    // inmediatamente a la version estatica incluida.
    const map = new Map(staticItems.map(item => [item.filename, item]));

    records.forEach(record => {
      const documentState = stateDocument(record.state);
      const staticBase = map.get(record.filename) || null;
      map.set(record.filename, {
        ...
        static: false,
        deletable: true,
        hasStaticBase: Boolean(staticBase),
        staticPath: staticBase?.path || ""
      });
    });

    return [...map.values()].sort((left, right) =>
      left.title.localeCompare(right.title, "es", { sensitivity: "base" }) ||
      left.filename.localeCompare(right.filename, "es", { sensitivity: "base" })
    );
  }

  async function getSong(filename) {
    const record = await getRecord(`song:${filename}`);
    if (record?.state !== undefined) return { state: record.state, source: "local" };
    const state = await loadStaticSong(filename);
    return state ? { state, source: "static" } : null;
  }
  ...
  async function routeApi(path, method, input, init) {
    ...
    if (path.startsWith("/api/songs/") && method === "GET") {
      ...
      return result ? jsonResponse(result.state) : jsonResponse({ error: "No existe ese analisis" }, 404);
    }
    if (path.startsWith("/api/songs/") && method === "DELETE") {
      const filename = safeJsonFilename(decodeURIComponent(path.replace("/api/songs/", "")));
      const record = await getRecord(`song:${filename}`);
      if (!record) {
        const staticSong = await findStaticSong(filename);
        if (staticSong) {
          return jsonResponse({
            error: "La cancion pertenece a la biblioteca incluida y no se puede borrar desde el navegador."
          }, 409);
        }
        return jsonResponse({ error: "No existe ese analisis" }, 404);
      }
      await deleteRecord(`song:${filename}`);
      return jsonResponse({ ok: true, restoredStaticBase: Boolean(await findStaticSong(filename)) });
    }
