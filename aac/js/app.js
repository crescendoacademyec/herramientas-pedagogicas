...
function updateCopySaveDestinationHint() {
  ...
  els.copySaveDestinationHint.textContent = destination === "curated"
    ...
    : "Aparecera en Archivo > Biblioteca... > Guardados. En la version web se guarda de forma privada en este navegador.";
}
...
async function refreshLibrary() {
  ...
  try {
    ...
    if (els.libraryList) els.libraryList.innerHTML = `<p class="libraryHint">No se pudo leer la biblioteca. Revisa data/library-index.json y las rutas de library / library2.</p>`;
  }
}
...
function renderLibrary(items, query = "") {
  ...
  const terms = librarySearchTerms(query);
  const matches = allItems.filter(item => itemMatchesTerms(item, terms, ["title", "composer", "filename"]));
  const DISPLAY_LIMIT = 180;
  const visible = matches.slice(0, DISPLAY_LIMIT);

  if (els.libraryCount) {
    els.libraryCount.textContent = terms.length
      ? `${matches.length} de ${allItems.length}`
      : `${allItems.length}`;
  }

  if (!allItems.length) {
    els.libraryList.innerHTML = `<p class="libraryHint">No hay canciones en la biblioteca ni guardados privados.</p>`;
    return;
  }

  if (!matches.length) {
    els.libraryList.innerHTML = `<p class="libraryHint">No encontré canciones con esa búsqueda.</p>`;
    return;
  }

  if (matches.length > DISPLAY_LIMIT) {
    const hint = document.createElement("p");
    hint.className = "libraryHint";
    hint.textContent = `Mostrando ${DISPLAY_LIMIT} de ${matches.length}. Escribe en el buscador para acotar la lista.`;
    els.libraryList.appendChild(hint);
  }
  ...
  visible.forEach(item => {
    ...
    const sourceLabel = item.source === "local"
      ? (item.hasStaticBase ? "Copia privada · reemplaza temporalmente la incluida" : "Guardado privado")
      : "Biblioteca ATA incluida";
    btn.innerHTML = `
    ...
    if (item.deletable !== false && item.source !== "static") {
      const deleteBtn = document.createElement("button");
      deleteBtn.className = "libraryDeleteBtn";
      deleteBtn.type = "button";
      deleteBtn.textContent = item.hasStaticBase ? "Quitar copia" : "Eliminar";
      deleteBtn.title = item.hasStaticBase
        ? `Quitar copia privada de ${item.filename}`
        : `Eliminar guardado privado ${item.filename}`;
...
async function deleteLibrarySong(item) {
  ...
  const deleteMessage = item?.hasStaticBase
    ? `Quitar copia privada:\n\n${item.title || filename}\n\nArchivo: ${filename}\n\nLa canción original incluida en library/library2 volverá a aparecer.`
    : `Eliminar guardado privado:\n\n${item.title || filename}\n\nArchivo: ${filename}\n\nSolo se borra la copia almacenada en este navegador.`;
  ...
  try {
    ...
    console.warn(error);
    alert("No se pudo eliminar el guardado privado.");
    return;
  }

  if (!res.ok) {
    let detail = "";
    try { detail = (await res.clone().json())?.error || ""; } catch (_) {}
    alert(`No se pudo eliminar el guardado privado.${detail ? `\n\n${detail}` : ""}`);
    return;
  }
  ...
  await refreshLibrary();
  alert(item?.hasStaticBase
    ? `Copia privada eliminada.\n\nSe restauró la canción incluida:\n${filename}`
    : `Guardado privado eliminado.\n\n${filename}`);
}
