...
// Suaviza la secuencia de acordes cuadro-a-cuadro con un filtro de "voto de mayoría" (moda)
// en una ventana deslizante, ponderado por la fuerza de detección de cada cuadro. Esto
// elimina el parpadeo de detecciones erróneas (uno o varios cuadros seguidos con un acorde
// distinto por ambigüedad armónica momentánea — p. ej. un bajo pasajero o un armónico que
// confunde al algoritmo) ANTES de construir los segmentos. Es mucho más efectivo que fusionar
// segmentos cortos después de construirlos, porque también corrige acordes espurios que
// alcanzan a durar varios cuadros y que antes sobrevivían al filtro de duración mínima,
// apareciendo como "acordes de más" que en realidad no se escuchan en la canción.
function smoothChordSequence(chords, strengths, windowSize) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = from; j < to; j++) {
      const c = chords[j];
      const w = Math.abs(strengths[j] || 0) + 0.0001; // pondera por confianza, nunca cero
      votes[c] = (votes[c] || 0) + w;
    }
    let bestChord = chords[i], bestVote = -1;
    Object.keys(votes).forEach((c) => { if (votes[c] > bestVote) { bestVote = votes[c]; bestChord = c; } });
    smoothed[i] = bestChord;
  }
  return smoothed;
}

// Segundo filtro: fusiona segmentos ADYACENTES que terminaron con el mismo acorde — esto pasa
// cuando un acorde espurio de corta duración quedó "en medio" (p. ej. A, B-espurio, A), ya que
// fusionar B dentro del segmento anterior deja dos tramos consecutivos de "A" en vez de uno solo.
function mergeAdjacentSameChord(segments) {
  const out = [];
  segments.forEach((seg) => {
    const prev = out[out.length - 1];
    if (prev && prev.chord === seg.chord) {
      const prevDur = prev.end - prev.start, segDur = seg.end - seg.start;
      const totalDur = prevDur + segDur || 1;
      prev.strength = parseFloat((((prev.strength * prevDur) + (seg.strength * segDur)) / totalDur).toFixed(3));
      prev.confidence = parseFloat(Math.min(prev.strength * 2, 1).toFixed(2));
      prev.end = seg.end;
    } else {
      out.push({ ...seg });
    }
...
function buildChordTimeline(chords, strengths, sampleRate, hopSize) {
  ...
  for (let i = 0; i < smoothedChords.length; i++) {
    const chord = smoothedChords[i];
