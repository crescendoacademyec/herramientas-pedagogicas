...
export function renderScore(state) {
  ...
  const barlines = measures.map((measure, index) => {
    ...
    return `<g class="measure" data-measure="${index}"><line x1="${x}" x2="${x}" y1="${STAFF_TOP}" y2="${STAFF_TOP + 4 * STAFF_GAP}" class="barline" /><text x="${x + 10}" y="${STAFF_TOP - 23}" class="measure-number">${measure.number}</text>${notes}</g>`;
  }).join("");
  ...
  return `<svg viewBox="0 0 ${endX + 42} 240" class="score-svg" role="img" aria-label="Partitura editable">
    ...
    <text x="54" y="${STAFF_TOP + 46}" class="clef">𝄞</text>
