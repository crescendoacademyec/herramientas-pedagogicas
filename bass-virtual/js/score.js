    ...
    function scoreCellKey(cell){ return `${cell.string}:${cell.fret}:${cell.midi}`; }
    ...
    function allPositionsForMidi(midi){
      ...
      for(let s=0;s<pitches.length;s++){
        const fret=midi-pitches[s]-activeInstrument().nutFret;
        if(Number.isInteger(fret)&&fret>=0&&fret<=maxDisplay){
    ...
    function attachMusicXmlHintsToSteps(){
      ...
      const octaveOffset=SCORE_OCTAVE_OFFSET[instrumentSel.value]||0;
      let hi=0;
    ...
    function bassFrettingModeFor(handPos){
      const explicit=bassFrettingModeSel?.value||'auto';
      if(explicit!=='auto') return explicit;
      // En registros graves la escala física es mayor: una digitación 1-2-4 evita
      // forzar innecesariamente el tercer dedo. Desde aprox. VII posición, OFPF resulta natural.
      return handPos<=6?'124':'ofpf';
    }
    function bassFingerForFret(fret,handPos,mode){
      if(fret===0)return 0;
    ...
    function fingerize(cells,handPos){
      let penalty=0;
      const mode=bassFrettingModeFor(handPos);
      const out=cells.map(c=>{
        if(c.fret===0)return {...c,finger:0,handPos,bassMode:mode};
        const raw=c.fret-handPos;
        const finger=bassFingerForFret(c.fret,handPos,mode);
        if(raw<0)penalty+=(-raw)*5;
        const maxSpan=mode==='124'?2:3;
        if(raw>maxSpan)penalty+=(raw-maxSpan)*6;
        // 1-2-4 no debe presentar el dedo 3 como digitación estable en posición grave.
        if(mode==='124'&&finger===3)penalty+=8;
        return {...c,finger,handPos,bassMode:mode};
      });
    ...
    function optimizePhraseSteps(steps){
      ...
      path.forEach((st,i)=>{
        st.annotations=[];
        if(st.cells?.length){const bm=st.cells.find(c=>c.bassMode)?.bassMode;if(bm==='124')st.annotations.push('digitación 1-2-4');else if(bm==='ofpf')st.annotations.push('1 dedo por traste');}
        const prev=path[i-1];
        if(prev){
    ...
    // El bajo eléctrico se escribe habitualmente una octava por encima de su sonido real
    ...
    window.addEventListener('load', function() {
      ...
      instrumentSel.value = (s.instrument&&INSTRUMENTS[s.instrument])?s.instrument:'bass4';
      ...
      if(bassFrettingModeSel&&['auto','ofpf','124'].includes(s.bassFrettingMode))bassFrettingModeSel.value=s.bassFrettingMode;
      ...
      if(bassExerciseSel&&['none','r5o','arpeggio1','pentatonic','horizontal'].includes(s.bassExercise))bassExerciseSel.value=s.bassExercise;
      if(rightHandPatternSel&&['free','im','mi','pick'].includes(s.rightHandPattern))rightHandPatternSel.value=s.rightHandPattern;
      if(exerciseDirectionSel&&['asc','desc','updown'].includes(s.exerciseDirection))exerciseDirectionSel.value=s.exerciseDirection;
      refresh();
      updateBassExercisePanel();
      drawBaseStaff();
      renderStaffNotes([]);
    });
...
[instrumentSel,tuningSel,capoSel,orientationSel,posSel,patternSel,bassFrettingModeSel].filter(Boolean).forEach(el=>el&&el.addEventListener('change',()=>{previousRecommendedCells=[];if(scoreHighlightMidis.size){updateFingeringForMidis([...scoreHighlightMidis],{remember:false});draw();}}));


// Fase 4 · si cambia el instrumento o su geometría, reconstruir el plan completo de frase.
[instrumentSel,tuningSel,capoSel,orientationSel,posSel,patternSel,bassFrettingModeSel].filter(Boolean).forEach(el=>el&&el.addEventListener('change',()=>{
